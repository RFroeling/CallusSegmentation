...
// File selection
inputDir = getDirectory("Select raw LIF directory");
fileList = getFileList(inputDir);
lifFiles = newArray();
for (i = 0; i < lengthOf(fileList); i++) {
    if (endsWith(fileList[i], ".lif")) {
        lifFiles = Array.concat(lifFiles, fileList[i]);
    }
}
print("Found " + lengthOf(lifFiles) + " LIF files in directory.\n");
...
for (j = 0; j < lengthOf(lifFiles); j++) {
    lifPath = inputDir + lifFiles[j];

    print("Processing file: " + lifFiles[j]);
